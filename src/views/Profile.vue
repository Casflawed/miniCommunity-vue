...
export default {
  ...
  methods: {
    getData() {
      ...
      if (!this.IsMe)
        this.$http
          .get("/profile/findUserProfile/" + this.$route.params.username)     //由userId不用，我服了
          .then((res) => {
  ...
  beforeRouteUpdate(to, from, next) {
    next();
    if (to.params.username === this.$store.getters.getUser.username) {//还是当前用户，其中vm还是原来的profile，因为会复用
